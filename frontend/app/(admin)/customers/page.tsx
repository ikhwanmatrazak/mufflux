...
export default function CustomersPage() {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-4">
        <Input placeholder="Search customers..." startContent={<Search size={16} className="text-foreground/30" />}
          ...
          <TableBody emptyContent={<p className="text-foreground/30 py-8">No customers found</p>}>
            {filtered.map((u) => (
              <TableRow key={u.id} className="border-b border-[#1a1a1a]">
                ...
                <TableCell className="text-foreground/40 text-sm">{new Date(u.created_at).toLocaleDateString()}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
