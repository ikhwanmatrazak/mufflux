...
export default function OrdersPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      <h1 className="text-2xl font-black text-foreground mb-6">{t("account.myOrders")}</h1>
      {orders.length === 0 ? (
        ...
        <div className="space-y-4">
          {orders.map((order) => (
            <div key={order.id} className="bg-content1 border border-divider rounded-2xl p-5 flex items-center gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  <p className="text-foreground font-bold">Order #{order.id}</p>
              ...
              <Link href={`/account/orders/${order.id}`}>
                <Button size="sm" variant="bordered" className="border-divider text-foreground/60">{t("account.viewDetail")}</Button>
              </Link>
            </div>
          ))}
        </div>
